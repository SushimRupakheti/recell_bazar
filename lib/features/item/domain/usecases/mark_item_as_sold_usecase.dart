...
final markItemAsSoldUsecaseProvider = Provider<MarkItemAsSoldUsecase>((ref) {
  final itemRepository = ref.read(itemRepositoryProvider);
  return MarkItemAsSoldUsecase(itemRepository: itemRepository);
});

class MarkItemAsSoldUsecase
    implements UsecaseWithParams<bool, String> {
  final IItemRepository _itemRepository;

  MarkItemAsSoldUsecase({required IItemRepository itemRepository})
      : _itemRepository = itemRepository;

  @override
  Future<Either<Failure, bool>> call(String itemId) {
    return _itemRepository.markItemAsSold(itemId);
  }
}
